%% All table generation script with computation times (max. heritability)
% 
...
% and prevalences 0.1 and 0.8. This version maximizes heritability.
%
% Warning: some models could not be computed to the given MAFs and
% prevalences combination. Warnings are raised to explicit this.
%
...
% Create a list of MAFs and prevalences to test
maf = [0.1, 0.4];
p = [0.1, 0.8];
    ...
    for j = 1:length(maf)
        ...
        for k = 1:length(p)
            try
                ...
                fprintf(fid, "%s_%.1f_p%.1f: %f\n", m.name, maf(j), p(k), comptime);
                fclose(fid);
            catch ME
                warning("(%s with MAF=%.1f and p=%.1f) %s", m.name, maf(j), p(k), ME.message);
                continue;
            end
        end
    end
